...
public class AppPurchaseWebhook {
    ...
    public void messageReceiver(String payload) throws Exception {
        ...
        if (payload.contains("oneTimeProductNotification")) {
            ...
            // for testing purpose
//            if (!userId.equals("hBEKR1PeXANTJrnTRbxAF8O8fVp2")
//                    && !userId.equals("UckBDr37V5WM41791rLbApsXCbM2")
//            ) {
//                return;
//            }
            ...
            switch (oneTimeProductNotification.getNotificationType()) {
                case 1:
                    ...
                    try {
                        ...
                        pdLogger.logException(PdLogger.Priority.p1, new Exception("successfully purchased by a user but Failed to add leafs, transactionId: " + purchaseToken));
                ...
                case 2:
                    ...
                    try {
                        ...
                        pdLogger.logException(PdLogger.Priority.p1, new Exception("User requested refund via GG Play Store but Failed to deduct leafs, transactionId: " + purchaseToken));
                        pdLogger.logException(e);
                    } finally {
                        break;
                    }
            ...
            switch (voidedPurchaseNotification.getRefundType()) {
                case 1:
                    ...
                    try {
                        ...
                        pdLogger.logException(PdLogger.Priority.p1, new Exception("Failed to deduct leafs for voided purchase, transactionId: " + purchaseToken));
                        pdLogger.logException(e);
                    } finally {
                        break;
                    }
